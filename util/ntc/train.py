...
parser.add_argument('--extract', type=str, nargs='+', required=True, help="Channels to extract per texture ('rgb', 'r', 'g', 'b')")
...
if not os.path.isfile(args.out):
    ...
    def load_texture(path, extract_str, flip=False):
        img = Image.open(path)
    
        if flip:
            img = img.transpose(Image.FLIP_TOP_BOTTOM)
    
        # Convert to RGB to ensure we always have 3 channels to slice from
        img = img.convert("RGB")
        tensor = TF.to_tensor(img).to(device)  # [3, H, W]
    
        #Explicitly slice the channels C++ asked for
        if extract_str == 'rgb':
            return tensor
        elif extract_str == 'r':
            return tensor[0:1, :, :]
    ...
    loaded_textures = []
    for path, ch, ext in zip(args.textures, args.channels, args.extract):
        tex = load_texture(path, ext, flip=args.flip)
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            if mip == 0:
                rgb_pred = pred[:, 0:3]
                rgb_y = y[:, 0:3]
                avg_psnr += compute_psnr(rgb_pred, rgb_y)
                psnr_count += 1
    ...
    with torch.no_grad():
        ...
        base_colour_tex = loaded_textures[0].cpu().clamp(0.0, 1.0)  # [3, H, W]
        base_colour_np = base_colour_tex.permute(1, 2, 0).numpy()   # [H, W, 3]
        base_colour_np = (base_colour_np * 255).astype(np.uint8)
        base_colour_img = Image.fromarray(base_colour_np, mode='RGB')
        base_colour_out = os.path.splitext(args.out)[0] + "_base_colour_debug.png"
        base_colour_img.save(base_colour_out)
        print(f"[Debug] Saved base colour texture to: {base_colour_out}")
    
    
    model.eval()
    with torch.no_grad():
        offset, size = input_chain_lookup[0]
        full_uvs = input_chain[offset:offset+size].to(device)
    
        # Run in chunks to avoid OOM
        chunk_size = 65536
        pred_chunks = []
        for i in range(0, full_uvs.shape[0], chunk_size):
            chunk = full_uvs[i:i+chunk_size]
            pred_chunks.append(model(chunk, 0).float().cpu())
        pred = torch.cat(pred_chunks, dim=0)  # [H*W, C]
    
        res = int(math.sqrt(size))
        pred_img = pred[:, 0:3].reshape(res, res, 3).clamp(0, 1).numpy()
        pred_img = (pred_img * 255).astype(np.uint8)
        pred_pil = Image.fromarray(pred_img, mode='RGB')
        pred_out = os.path.splitext(args.out)[0] + "_base_colour_reconstructed.png"
        pred_pil.save(pred_out)
        print(f"[Debug] Saved reconstructed base colour to: {pred_out}")