...
int main()
{
	...
	surfaceDesc.name = "Cube Sample";
	constexpr glm::ivec2 SCREEN_DIMENSIONS{ 1280, 720 };
	surfaceDesc.size = glm::ivec2(SCREEN_DIMENSIONS.x, SCREEN_DIMENSIONS.y);
	...
	camDataBufferDesc.size = 2 * sizeof(glm::mat4); //view matrix + proj matrix
	...
	glm::mat4 projMat{ glm::perspective(glm::radians(90.0f), static_cast<float>(SCREEN_DIMENSIONS.x) / SCREEN_DIMENSIONS.y, 0.01f, 100.0f) };
	glm::mat4 camMatrices[]{ viewMat, projMat };
	gpuUploader->EnqueueBufferDataUpload(camMatrices, camDataBuffer.get(), NK::RESOURCE_STATE::UNDEFINED);


	//Camera Data Buffer View
	NK::BufferViewDesc camDataBufferViewDesc{};
	camDataBufferViewDesc.size = 2 * sizeof(glm::mat4);
	...
	{
		//Positions								//Colours
		Vertex(glm::vec3( 0.5f, -0.5f,  0.5f),	glm::vec3(1.0f, 0.0f, 0.0f)), //0: Front Bottom Left	(Red)
		Vertex(glm::vec3(-0.5f, -0.5f,  0.5f),	glm::vec3(0.0f, 1.0f, 0.0f)), //1: Front Bottom Right	(Green)
		Vertex(glm::vec3(-0.5f,  0.5f,  0.5f),	glm::vec3(0.0f, 0.0f, 1.0f)), //2: Front Top Right		(Blue)
		Vertex(glm::vec3( 0.5f,  0.5f,  0.5f),	glm::vec3(1.0f, 1.0f, 0.0f)), //3: Front Top Left		(Yellow)
		Vertex(glm::vec3( 0.5f, -0.5f, -0.5f),	glm::vec3(1.0f, 0.0f, 1.0f)), //4: Back Bottom Left		(Magenta)
		Vertex(glm::vec3(-0.5f, -0.5f, -0.5f),	glm::vec3(0.0f, 1.0f, 1.0f)), //5: Back Bottom Right	(Cyan)
		Vertex(glm::vec3(-0.5f,  0.5f, -0.5f),	glm::vec3(1.0f, 1.0f, 1.0f)), //6: Back Top Right		(White)
		Vertex(glm::vec3( 0.5f,  0.5f, -0.5f),	glm::vec3(0.5f, 0.5f, 0.5f))  //7: Back Top Left		(Gray)
	};
