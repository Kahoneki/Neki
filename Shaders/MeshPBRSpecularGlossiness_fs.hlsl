...
[[vk::binding(0,0)]] ConstantBuffer<NK::PBRSpecularGlossinessMaterial> g_materials[] : register(b0, space0);
...
{
	NK::PBRSpecularGlossinessMaterial material = g_materials[NonUniformResourceIndex(PC(materialBufferIndex))];
	...
	float3 diffuseSample = float3(1, 1, 1);
	if (material.hasDiffuse) { diffuseSample = g_textures[NonUniformResourceIndex(material.diffuseIdx)].Sample(linearSampler, vertexOutput.texCoord).rgb; }
	else { return float4(1, 0, 1, 1); }
	
	float3 specularSample = float3(0.04f, 0.04f, 0.04f);
	if (material.hasSpecular) { specularSample = g_textures[NonUniformResourceIndex(material.specularIdx)].Sample(linearSampler, vertexOutput.texCoord).rgb; }

	float glossinessSample = 1.0f;
	if (material.hasGlossiness) { glossinessSample = g_textures[NonUniformResourceIndex(material.glossinessIdx)].Sample(linearSampler, vertexOutput.texCoord).r; }
	...
	float ao = 1.0f;
	if (material.hasAO) { ao = g_textures[NonUniformResourceIndex(material.aoIdx)].Sample(linearSampler, vertexOutput.texCoord).r; }
	...
	float3 F0 = specularSample;
	float maxSpec = max(specularSample.r, max(specularSample.g, specularSample.b));
	float3 albedo = diffuseSample * (1.0f - maxSpec);
	float roughness = 1.0f - glossinessSample;
	float metallic = 0.0f;
	...
	//Add the variance to the roughness (the further away (i.e.: the steeper the angle), the rougher it gets)
	float minRoughness = 0.04f; //don't let roughness be zero to prevent nans
